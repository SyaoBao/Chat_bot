...
import numpy as np
import matplotlib 
# Установка неинтерактивного бэкенда (ДОЛЖНО БЫТЬ ДО import pyplot!)
matplotlib.use('Agg')  # ← Критически важно! Неинтерактивный бэкенд Anti-Grain Geometry (основной)
...
# 2. Укажите ваш токен для доступа к HTTP API:
API_TOKEN = 'Укажите ваш токен'
bot = telebot.TeleBot(API_TOKEN)

# 3. Работа с датами и данными
...
# 10. Генерация торговых рекомендаций и расчета прибыли
...
# 11. Модуль взаимодействия с пользователем чат-бота. Обрабатывает все входящие команды и сообщения.
...
⚠️ **Важно:** Это учебный прогноз, не является финансовой рекомендацией!

🤖 **Бот для анализа и прогнозирования акций**

Привет! Я робот на основе нейронной сети.

Я помогу вам получить прогноз цен акций и рекомендации по торговым стратегиям.

**Как использовать:**
1. Введите команду: `/choose TICKER СУММА`
2. Например: `/choose AAPL 10000`
...
Полный список тикеров: https://finance.yahoo.com/

После команды `/choose TICKER СУММА` бот автоматически загрузит исторические данные о стоимости акций (за последние 2 года), обучит несколько моделей временных рядов, выберет наилучшую по метрикам качества и построит прогноз на ближайшие 30 дней.
Необходимо немного подождать, пока нейронная сеть будет обучаться и создавать прогноз.
...
def handle_exit(message):
    ...
    try:
        ...
        goodbye_text = f"""
👋 **Спасибо за использование бота! {user_name}!**
